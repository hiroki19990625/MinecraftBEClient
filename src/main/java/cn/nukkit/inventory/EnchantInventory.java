...
public class EnchantInventory extends ContainerInventory {
    ...
    public void onSlotChange(int index, Item before, boolean send) {
        ...
        if (index == 0) {
            ...
            if (item.getBlockId() == Item.AIR) {
                this.entries = null;
            } else if (before.getBlockId() == Item.AIR && !item.hasEnchantments()) {
                //before enchant
    ...
    public void onEnchant(Player who, Item before, Item after) {
        Item result = (before.getBlockId() == Item.BOOK) ? new ItemBookEnchanted() : before;
        if (!before.hasEnchantments() && after.hasEnchantments() && after.getBlockId() == result.getBlockId() && this.levels != null && this.entries != null) {
            ...
            for (int i = 0; i < 3; i++) {
                if (Arrays.equals(enchantments, this.entries[i].getEnchantments())) {
                    ...
                    if (lapis.getBlockId() == Item.DYE && lapis.getDamage() == DyeColor.BLUE.getDyeData() && lapis.getCount() > i && level >= cost) {
                ...
                for (int z = -1; z <= 1; z++) {
                    ...
                    if (level.getBlock(loc.add(x, 0, z)).isTransparent()) {
                        if (level.getBlock(loc.add(0, 1, 0)).isTransparent()) {
                            ...
                            if (level.getBlock(loc.add(x << 1, y, z << 1)).getBlockId() == Block.BOOKSHELF) {
                                count++;
                            }

                            if (x != 0 && z != 0) {
                                //one block diagonal and one straight
                                if (level.getBlock(loc.add(x << 1, y, z)).getBlockId() == Block.BOOKSHELF) {
                                    ++count;
                                }

                                if (level.getBlock(loc.add(x, y, z << 1)).getBlockId() == Block.BOOKSHELF) {
                                    ++count;
                                }
                            }
                        }
