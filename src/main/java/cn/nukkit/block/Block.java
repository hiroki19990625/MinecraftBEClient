...
import itsu.java.minecraftbeclient.block.Texture;
...
public abstract class Block extends Position implements Metadatable, Cloneable, AxisAlignedBB, BlockID {
    ...
    public Item[] getDrops(Item item) {
        if (this.getBlockId() < 0 || this.getBlockId() > list.length) { //Unknown blocks
            return new Item[0];
        } else {
            return new Item[]{
                    this.toItem()
    ...
    public String toString() {
        return "Block[" + this.getName() + "] (" + this.getBlockId() + ":" + this.getDamage() + ")";
    }
    ...
    public static boolean equals(Block b1, Block b2, boolean checkDamage) {
        return b1 != null && b2 != null && b1.getBlockId() == b2.getBlockId() && (!checkDamage || b1.getDamage() == b2.getDamage());
    }
