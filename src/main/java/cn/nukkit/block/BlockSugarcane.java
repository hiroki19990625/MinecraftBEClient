...
public class BlockSugarcane extends BlockFlowable {
        ...
        if (item.getId() == Item.DYE && item.getDamage() == 0x0F) { //Bonemeal
            ...
            if (count < 3) {
                ...
                for (int i = 1; i <= toGrow; i++) {
                    ...
                    } else if (block.getBlockId() != SUGARCANE_BLOCK) {
                        break;
                    }
                }
            }
    ...
    public int onUpdate(int type) {
        if (type == Level.BLOCK_UPDATE_NORMAL) {
            ...
            if (down.isTransparent() && down.getBlockId() != SUGARCANE_BLOCK) {
                this.getLevel().useBreakOn(this);
                return Level.BLOCK_UPDATE_NORMAL;
            }
        } else if (type == Level.BLOCK_UPDATE_RANDOM) {
            if (this.down().getBlockId() != SUGARCANE_BLOCK) {
    ...
    public boolean place(Item item, Block block, Block target, BlockFace face, double fx, double fy, double fz, Player player) {
        ...
        Block down = this.down();
        if (down.getBlockId() == SUGARCANE_BLOCK) {
            this.getLevel().setBlock(block, new BlockSugarcane(), true);
            return true;
        } else if (down.getBlockId() == GRASS || down.getBlockId() == DIRT || down.getBlockId() == SAND) {
