...
public class BlockDoublePlant extends BlockFlowable {
    ...
    public int onUpdate(int type) {
        if (type == Level.BLOCK_UPDATE_NORMAL) {
            if ((this.getDamage() & 0x08) == 8) {
                // Top
                if (!(this.down().getBlockId() == DOUBLE_PLANT)) {
                    this.getLevel().setBlock(this, new BlockAir(), false, true);
                    return Level.BLOCK_UPDATE_NORMAL;
                }
            } else {
                // Bottom
                if (this.down().isTransparent() || !(this.up().getBlockId() == DOUBLE_PLANT)) {
    ...
    public boolean place(Item item, Block block, Block target, BlockFace face, double fx, double fy, double fz, Player player) {
        ...
        if (up.getBlockId() == 0 && (down.getBlockId() == GRASS || down.getBlockId() == DIRT)) {
