...
public abstract class BlockDoor extends BlockTransparentMeta {
    ...
    public boolean place(Item item, Block block, Block target, BlockFace face, double fx, double fy, double fz, Player player) {
        if (face == BlockFace.UP) {
            ...
            int metaUp = 0x08;
            if (left.getBlockId() == this.getBlockId() || (!right.isTransparent() && left.isTransparent())) { //Door hinge
                metaUp |= 0x01;
            }

            this.setDamage(direction);
            this.getLevel().setBlock(block, this, true, true); //Bottom
            this.getLevel().setBlock(blockUp, Block.get(this.getBlockId(), metaUp), true); //Top
    ...
    public boolean onBreak(Item item) {
        if ((this.getDamage() & 0x08) == 0x08) {
            ...
            if (down.getBlockId() == this.getBlockId()) {
                this.getLevel().setBlock(down, new BlockAir(), true);
            }
        } else {
            Block up = this.up();
            if (up.getBlockId() == this.getBlockId()) {
    ...
    public boolean toggle(Player player) {
        ...
        if (isTop(this.getDamage())) { //Top
            ...
            this.getLevel().setBlock(down, Block.get(this.getBlockId(), down.getDamage() ^ 0x04), true);
