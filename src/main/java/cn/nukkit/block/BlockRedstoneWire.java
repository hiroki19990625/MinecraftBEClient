...
public class BlockRedstoneWire extends BlockFlowable {
    ...
    private void updateAround(Vector3 pos, BlockFace face) {
        if (this.level.getBlock(pos).getBlockId() == Block.REDSTONE_WIRE) {
    ...
    private int getMaxCurrentStrength(Vector3 pos, int maxStrength) {
        if (this.level.getBlockIdAt(pos.getFloorX(), pos.getFloorY(), pos.getFloorZ()) != this.getBlockId()) {
            return maxStrength;
        } else {
    ...
    public boolean canBePlacedOn(Vector3 v) {
        ...
        return b.isSolid() && !b.isTransparent() && b.getBlockId() != Block.GLOWSTONE;
    }
    ...
    protected static boolean canConnectTo(Block block, BlockFace side) {
        if (block.getBlockId() == Block.REDSTONE_WIRE) {
            return true;
