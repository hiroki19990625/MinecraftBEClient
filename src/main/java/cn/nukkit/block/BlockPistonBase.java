    ...
    private void checkState() {
        ...
        if (isPowered && !isExtended()) {
            ...
            if (this.sticky) {
                ...
                if (block.getBlockId() == AIR) {
                    this.level.setBlock(this.getLocation().getSide(facing), new BlockAir(), true, true);
                }
                if (canPush(block, facing.getOpposite(), false) && (!(block instanceof BlockFlowable) || block.getBlockId() == PISTON || block.getBlockId() == STICKY_PISTON)) {
                    this.doMove(false);
                }
            } else {
        ...
        public boolean canMove() {
            ...
            if (!canPush(block, this.moveDirection, false)) {
                ...
                for (int i = 0; i < this.toMove.size(); ++i) {
                    ...
                    if (b.getBlockId() == SLIME_BLOCK && !this.addBranchingBlocks(b)) {
                        return false;
                    }
                }
        ...
        private boolean addBlockLine(Block origin) {
            ...
            if (block.getBlockId() == AIR) {
                ...
                if (count + this.toMove.size() > 12) {
                    ...
                    while (block.getBlockId() == SLIME_BLOCK) {
                        block = origin.getSide(this.moveDirection.getOpposite(), count);

                        if (block.getBlockId() == AIR || !canPush(block, this.moveDirection, false) || block.equals(this.pistonPos)) {
                            break;
                        }
