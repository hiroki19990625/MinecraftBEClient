...
public class BlockBed extends BlockTransparentMeta {
    ...
    public boolean onActivate(Item item, Player player) {
        ...
        if ((this.getDamage() & 0x08) == 0x08) {
            ...
            if (blockNorth.getBlockId() == this.getBlockId() && (blockNorth.getDamage() & 0x08) == 0x08) {
                b = blockNorth;
            } else if (blockSouth.getBlockId() == this.getBlockId() && (blockSouth.getDamage() & 0x08) == 0x08) {
                b = blockSouth;
            } else if (blockEast.getBlockId() == this.getBlockId() && (blockEast.getDamage() & 0x08) == 0x08) {
                b = blockEast;
            } else if (blockWest.getBlockId() == this.getBlockId() && (blockWest.getDamage() & 0x08) == 0x08) {
                b = blockWest;
            } else {
                if (player != null) {
    ...
    public boolean place(Item item, Block block, Block target, BlockFace face, double fx, double fy, double fz, Player player) {
        ...
        if (!down.isTransparent()) {
            ...
            if (next.canBeReplaced() && !downNext.isTransparent()) {
                ...
                this.getLevel().setBlock(block, Block.get(this.getBlockId(), meta), true, true);
                this.getLevel().setBlock(next, Block.get(this.getBlockId(), meta | 0x08), true, true);
    ...
    public boolean onBreak(Item item) {
        ...
        if ((this.getDamage() & 0x08) == 0x08) { //This is the Top part of bed
            if (blockNorth.getBlockId() == this.getBlockId() && blockNorth.getDamage() != 0x08) { //Checks if the block ID&&meta are right
                this.getLevel().setBlock(blockNorth, new BlockAir(), true, true);
            } else if (blockSouth.getBlockId() == this.getBlockId() && blockSouth.getDamage() != 0x08) {
                this.getLevel().setBlock(blockSouth, new BlockAir(), true, true);
            } else if (blockEast.getBlockId() == this.getBlockId() && blockEast.getDamage() != 0x08) {
                this.getLevel().setBlock(blockEast, new BlockAir(), true, true);
            } else if (blockWest.getBlockId() == this.getBlockId() && blockWest.getDamage() != 0x08) {
