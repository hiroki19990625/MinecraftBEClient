...
//                        System.out.println("schedule update 2");
    ...
    protected int calculateInputStrength() {
        ...
        if (power >= 15) {
            ...
            return Math.max(power, block.getBlockId() == Block.REDSTONE_WIRE ? block.getDamage() : 0);
        }
    }
    ...
    protected int getPowerOnSide(Vector3 pos, BlockFace side) {
        Block block = this.level.getBlock(pos);
        return isAlternateInput(block) ? (block.getBlockId() == Block.REDSTONE_BLOCK ? 15 : (block.getBlockId() == Block.REDSTONE_WIRE ? block.getDamage() : this.level.getStrongPower(pos, side))) : 0;
    }
