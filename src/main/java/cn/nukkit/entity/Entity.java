...
public abstract class Entity extends Location implements Metadatable {
    ...
    public float getEyeHeight() {
        return this.getEntityHeight() / 2 + 0.1f;
    }
    ...
    public void setScale(float scale) {
        ...
        float height = this.getEntityHeight() * this.scale;
        double radius = (this.getEntityWidth() * this.scale) / 2d;
    ...
    public void spawnTo(Player player) {
        ...
        if (this.riding != null) {
            ...
            pkk.rider = this.riding.getEntityId();
            pkk.riding = this.getEntityId();
            pkk.type = 1;
            pkk.unknownByte = 1;
    ...
    protected DataPacket createAddEntityPacket() {
        ...
        addEntity.entityUniqueId = this.getEntityId();
        addEntity.entityRuntimeId = this.getEntityId();
    ...
    public void fall(float fallDistance) {
        ...
        if (fallDistance > 0.75) {
            ...
            if (down.getBlockId() == Item.FARMLAND) {
                Event ev;
    ...
    public boolean setPosition(Vector3 pos) {
        ...
        double radius = this.getEntityWidth() / 2d;

        this.boundingBox.setBounds(pos.x - radius, pos.y, pos.z - radius, pos.x + radius, pos.y + (this.getEntityHeight() * this.scale), pos.z
                + radius);
    ...
    public int hashCode() {
        int hash = 7;
        hash = (int) (29 * hash + this.getEntityId());
        return hash;
    }
}
