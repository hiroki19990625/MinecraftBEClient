...
//    private final List<BlockUpdateEntry> nextTickUpdates = Lists.newArrayList();
        ...
        if (optimizeRebuilds) {
            ...
            for (Vector3 b : blocks) {
                ...
                if (b instanceof Block) {
                    ...
                    updateBlockPacket.blockRuntimeId = GlobalBlockPalette.getOrCreateRuntimeId(((Block) b).getBlockId(), ((Block) b).getDamage());
    ...
    public void scheduleUpdate(Block block, Vector3 pos, int delay, int priority, boolean checkArea) {
        if (block.getBlockId() == 0 || (checkArea && !this.isChunkLoaded(block.getFloorX() >> 4, block.getFloorZ() >> 4))) {
            return;
        }
...
//        }
        ...
        if (tag instanceof ListTag) {
            ...
            for (Tag v : ((ListTag<Tag>) tag).getAll()) {
                if (v instanceof StringTag) {
                    ...
                    if (entry.getId() > 0 && entry.getBlock() != null && entry.getBlock().getBlockId() == target.getBlockId()) {
                        canBreak = true;
                        break;
                    }
                }
    ...
    public Item useItemOn(Vector3 vector, Item item, BlockFace face, float fx, float fy, float fz, Player player, boolean playSound) {
        ...
        if (!(block.canBeReplaced() || (hand.getBlockId() == Item.SLAB && block.getBlockId() == Item.SLAB))) {
            return null;
        }
        ...
        if (playSound) {
            this.addLevelSoundEvent(hand, LevelSoundEventPacket.SOUND_PLACE, 1, GlobalBlockPalette.getOrCreateRuntimeId(hand.getBlockId(), hand.getDamage()), false);
        }
    ...
    public void removeEntity(Entity entity) {
        ...
        if (entity instanceof Player) {
            this.players.remove(entity.getEntityId());
            this.checkSleep();
        } else {
            entity.close();
        }
        ...
        this.updateEntities.remove(entity.getEntityId());
    }
    ...
    public void addEntity(Entity entity) {
        ...
        if (entity instanceof Player) {
            this.players.put(entity.getEntityId(), (Player) entity);
        }
    ...
    public void addBlockEntity(BlockEntity blockEntity) {
        ...
        blockEntities.put(blockEntity.getBlockEntityId(), blockEntity);
    }
    ...
    public void removeBlockEntity(BlockEntity entity) {
        Preconditions.checkNotNull(entity, "entity");
        Preconditions.checkArgument(entity.getLevel() == this, "BlockEntity is not in this level");
        blockEntities.remove(entity.getBlockEntityId());
